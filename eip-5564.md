...
Furthermore, to counter spamming as a parsing provider, staking mechanism or the charging of a `toll` could be introduced. Staking mechanism may allow users to stake an unslashable amount of ETH (similarly to [ERC-4337](./eip-4337)), thereby assisting in better tackling potential spam through *sybil attacks* and enabling to filter spam more effectively.
...
Thus, the sender may attach a small amount of ETH to each stealth address transaction, thereby sponsering subsequent transactions of the recipient.
